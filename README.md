...
V4/figures/
  figures_data_v1.py    generates paper figures F1-F8 (threat trends, heatmap, ROC)
  fig_exp_ieee.py       IEEE-formatted experimental figures (E2, E4, E6, E7)
  fig_f8_ieee.py        IEEE two-column build for the threat-trends figure (F8)
  figures_tikz_v1.tex   TikZ source for conceptual architecture diagrams
  build/                pre-generated PDF/PNG/SVG outputs
tests/
...
The experiment runs entirely on **synthetic data** generated by `shield_ai/synthetic_data.py`,
which reproduces the statistical properties of CICIDS-2017 (24 features, 15 classes) without
requiring an external download. No dataset file needs to be downloaded to reproduce the paper
results -- simply run the quick-start commands above.

If you want to validate against the original CICIDS-2017 captures, download the CSV files from
the Canadian Institute for Cybersecurity (<https://www.unb.ca/cic/datasets/ids-2017.html>) and
