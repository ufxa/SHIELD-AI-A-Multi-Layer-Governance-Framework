...
The threshold $r^*_{\text{auto}}$ is sensitive to the ratio
$\kappa = c_{\text{err}} / (b + c_{\text{err}})$. For the reference configuration
($b{=}1$, $c_{\text{err}}{=}2$, $c_{\text{analyst}}{=}0.3$, $\lambda{=}0.3$),
$r^*_{\text{auto}} = 0.80$. Three boundary conditions of operational interest:
(i) when $c_{\text{err}} \to \infty$ (catastrophic false negative, e.g., critical
infrastructure shutdown), $r^*_{\text{auto}} \to 1 - (c_{\text{analyst}} + \lambda)/b$,
meaning only near-perfect confidence escapes HITL; (ii) when $c_{\text{analyst}} \to 0$
(automated HITL with negligible analyst cost), $r^*_{\text{auto}} \to 1 - \kappa$,
reducing the AUTO-eligible zone to near-certain detections only; (iii) when
$b \gg c_{\text{err}}$ (low error cost relative to benefit, e.g., informational alerts),
$r^*_{\text{auto}} \to 1 - c_{\text{analyst}}/b$, allowing aggressive AUTO routing even
at moderate confidence. These boundary analyses confirm that PC3 degrades gracefully
under parameter extremes and retains interpretable behavior across the full feasible
parameter space.
...
append and Merkle batch commitment (layer4\_audit.py, excerpt).},
label=lst:audit, float=tp]
...
class AuditLog:
    ...
    def _commit_batch(self) -> str:
        leaves = [r["sig"] for r in
                  self._records[-self._batch_size:]]
    ...
    def _merkle_root(leaves: list[str]) -> str:
        ...
        while len(layer) > 1:
            ...
            layer = [hashlib.sha256(
                         (a + b).encode()).hexdigest()
                     for a, b in
                     zip(layer[::2], layer[1::2])]
        return layer[0]
    ...
    def verify_chain(self) -> bool:
        ...
        for r in self._records:
            ...
            expected = hmac.new(
                self._secret, raw, hashlib.sha256
            ).hexdigest()
...
The healthcare SOC scenario highlights three SHIELD-AI properties that are absent from
conventional SIEM deployments. First, the PII gate in L1 prevents EHR patient identifiers
(CPF, CNS card) from being forwarded to the LLM prompt, maintaining LGPD Art.~11
restrictions on sensitive health data processing even within the security pipeline.
Second, the GraphRAG Knowledge Layer allows correlation of network anomalies with
medical device inventory (represented as Asset nodes in $\mathcal{G}$), enabling
identification of ransomware lateral movement via connected infusion pumps or PACS
servers -- correlations that vector-only RAG cannot surface. Third, when the L4
reliability composite falls below $r^*_{\text{auto}}$ for an alert tagged with LGPD
sensitivity labels, L5 routes to a clinician-aware analyst tier rather than the
standard IT analyst queue, ensuring that the human reviewer has domain context matching
the alert's sensitivity class. This tri-layer response -- PII isolation, graph-aware
correlation, and tiered HITL -- instantiates the dependability argument at the
fault-prevention, fault-tolerance, and fault-removal levels simultaneously.
...
In OT/SCADA environments, the latency penalty $\lambda$ in the PC3 cost model acquires
operational significance beyond analyst workflow: an unmitigated ransomware event in a
power-generation SCADA network can trigger physical process shutdowns within seconds of
payload activation. This converts $c_{\text{err}}$ from a financial to a physical-safety
cost parameter. SHIELD-AI operationalizes this by allowing sector-specific cost
parameterization without modifying the routing algorithm: operators set $c_{\text{err}}$
proportional to the estimated cost of false-negative misclassification in their specific
OT context (downtime cost per minute $\times$ expected incident duration), and
Theorem~\ref{thm:routing} guarantees that the derived threshold $r^*_{\text{auto}}$
is policy-optimal under those parameters.

Brazilian critical infrastructure SOCs face the additional regulatory dimension of
LGPD's Sector Resolution pipeline, which requires incident notification to the ANPD within
72 hours if personal data is involved. L6's Merkle-chained audit log provides the evidence
base for this notification: the log includes the PII flag decision from L1, the LLM's
GDPR/LGPD sensitivity assessment from L4, and the analyst's final classification from L5,
constituting the required chain of events for regulatory notification purposes. The
monotonicity guarantee (P2) ensures that adding ANPD-specific controls to the mapping
can only increase coverage, providing a compliance-by-construction argument for new
regulatory requirements as they are issued.
...
The computational cost of SHIELD-AI at inference time is dominated by three operations.
\emph{L3 GraphRAG retrieval}: given a knowledge graph with $|V|$ nodes and $|E|$ edges,
multi-hop traversal for $k$ hops requires $O(d^k)$ neighbor expansions, where $d$ is the
average node degree. In practice, $k \leq 3$ and $d < 50$ for SOC knowledge graphs of
typical size ($|V| \sim 10^4$), yielding sub-second latency per alert. Vectorized
batch retrieval further amortizes the cost across concurrent alerts.
\emph{L4 LLM inference}: scales linearly in prompt length. The structured prompt template
(Section~\ref{ssec:calibration}) bounds context to $\sim$2000 tokens per alert, keeping
per-alert latency predictable regardless of alert volume.
\emph{L6 audit logging}: each record requires one HMAC-256 operation and one dictionary
merge, both $O(1)$. Merkle batch commitment (one SHA-256 tree per batch of $B$ records)
costs $O(B \log B)$ and is performed asynchronously, incurring no latency on the alert path.

At scale, the principal bottleneck shifts to LLM API throughput. With a parallelism budget
of $P$ concurrent LLM calls, SHIELD-AI sustains $P \cdot (1/t_{\text{LLM}})$ alerts per
second, where $t_{\text{LLM}}$ is median per-alert latency (empirically 1.2 s in our
setup). For $P = 16$, this yields $\sim$13 alerts/s -- sufficient for mid-tier SOCs
ingesting $<$10$^6$ alerts/day. High-volume MSSPs should front the L4 tier with a
priority queue that routes only above-threshold-confidence alerts to the LLM, falling back
to the rule-based L1 classifier for burst absorption. This is consistent with the 69\%
AUTO routing rate observed in our benchmark: the majority of alerts never reach L4.

\subsection{Research Agenda}
\label{ssec:agenda}
Nine open problems frame the forward research programme.

\textbf{OP1 -- Operational validation.} The reference experiment uses synthetic data
derived from CICIDS-2017; live SOC validation via historical-incident replay is the
critical next step. Partnering with a MSSP or national CSIRT to annotate an incident
log and replay it through SHIELD-AI would yield empirical ground truth for the PC3
routing thresholds and per-class F1 scores.

\textbf{OP2 -- Multi-agent adversarial dynamics.} SHIELD-AI currently assumes a
single-agent architecture. The OWASP Agentic AI Top-10 \cite{owasp2025agentic}
documents emergent risks when agents delegate subtasks to subordinate agents, including
trust boundary collapse and prompt injection cascades \cite{lga2026}. Extending the
formal model (Definition~1) to multi-agent semantics, with explicit trust labels per
inter-agent message, is required before SHIELD-AI can govern autonomous AI pipelines.

\textbf{OP3 -- Cross-jurisdictional compliance.} The current control mapping covers
LGPD and GDPR. Extending it to CCPA (California), DPDP (India), and the forthcoming
ASEAN AI governance framework would enable SHIELD-AI deployment in multinational SOCs
and test the monotonicity guarantee (P2) under heterogeneous regulatory logics.

\textbf{OP4 -- Continuous-learning HITL.} Analyst corrections in L5 represent
a rich signal for online fine-tuning of the L4 LLM. Safe integration requires
distribution-shift detection (to halt fine-tuning when the correction stream diverges
from the training distribution) and audit-trail linkage (so every model update is
traceable to specific L5 decisions). Theorem~\ref{thm:routing} does not extend to
continuously shifting models without re-deriving the calibration bound.

\textbf{OP5 -- Cost-effectiveness analysis.} A full TCO comparison against the
baseline SOC configuration (without SHIELD-AI) requires instrumenting analyst-hours
per true positive, false-positive investigation cost, and audit preparation overhead.
The 69\% AUTO routing rate is a proxy, not a definitive cost reduction estimate.

\textbf{OP6 -- Standards co-evolution.} The seven standards in the mapping evolve
independently. A protocol for tracking version changes, flagging affected controls,
and re-verifying P2 after updates would reduce the maintenance burden for compliance
teams and is a prerequisite for certification-body adoption.

\textbf{OP7 -- Open-source reference implementation.} The companion repository
(\url{https://github.com/ufxa/SHIELD-AI-A-Multi-Layer-Governance-Framework})
contains the experiment runner and evaluation scripts. A camera-ready DOI-pinned
release with Docker-compose deployment, a synthetic alert dataset, and an interactive
HITL dashboard would lower the barrier for replication studies.

\textbf{OP8 -- Empirical calibration of reliability weights.} The composite reliability
$R = \omega_\theta\theta + \omega_\sigma\sigma + \omega_\gamma\gamma + \omega_\rho\rho$
uses uniform weights in the reference experiment. Empirical calibration across LLM
families (GPT-4o, Claude, Gemini, Llama-3) and alert taxonomies would determine whether
a single weight vector transfers across deployments or requires per-deployment tuning.

\textbf{OP9 -- Purple-team evaluation and KPI programme.} Adversarial purple-team
exercises applying prompt injection \cite{greshake2023indirect} and universal adversarial
suffixes \cite{zou2023universal} in a controlled SOC lab would stress-test the L4 controls
and HITL escalation path under adversarial conditions. Alongside this, a Compliance
Theater Index KPI -- measuring the gap between declared controls and operationalized
behavior -- would provide a quantitative mechanism for detecting governance-without-practice,
directly addressing limitation A7.
