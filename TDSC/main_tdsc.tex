...
\title{SHIELD-AI: A Multi-Layer Governance Reference Architecture for Safer and Auditable Generative AI Integration in Security Operations Centers}

\author{José~de~Souza~Junior,~Milton~S.~C.~de~Oliveira,~%
Jussiléa~Gurjão~de~Figueiredo,~Romualdo~A.~P.~Júnior,~%
Marcelo~A.~da~Silva,~and~\mbox{Allan~D.~B.~Costa}%
...
\thanks{M.~S.~C.~de~Oliveira is with the Universidade Nove de Julho (UNINOVE),
São Paulo, SP, Brazil, and also with the Technology and Information Company
of Social Security (DATAPREV), São Paulo, SP, Brazil
(e-mail: milton.oliveira@dataprev.gov.br).
ORCID: \href{https://orcid.org/0000-0002-7966-3960}{0000-0002-7966-3960}.}%
...
\thanks{R.~A.~P.~Júnior is with Modal Institute, Brasília, DF, Brazil
...
Our review identifies four governance gaps not jointly addressed by the artifacts examined:
...
To address them, we propose \textbf{SHIELD-AI}, a six-layer reference architecture for safer and
more auditable generative AI integration in security operations.
Grounded in Design Science Research and a focused mapping represented by 50 cited sources, SHIELD-AI
...
We formalize SHIELD-AI as a 6-tuple, derive an AUTO/HITL decision boundary, and state three
conditional properties concerning trace completeness, selected-control coverage, and
policy-derived routing.
A reproducible component-level experiment on a synthetic CICIDS-2017-like benchmark
($n{=}60{,}000$), using a Random-Forest-derived LLM/RAG surrogate rather than a production LLM,
yields binary F1$\,{=}\,0.984$, AUC$\,{=}\,0.998$, successful local audit-chain verification,
and a PC3 routing split of 69\%/30\%/1\% (AUTO/HITL/REJECT). These results demonstrate prototype
execution, not operational GenAI safety or field effectiveness.
\end{abstract}
...
Under this author-defined coding frame, SHIELD-AI is the only artifact marked as addressing
all eight. The table is a positioning aid rather than evidence of exhaustive novelty. The contribution is architectural
...
A focused literature mapping over 2023--2026 plus foundational classics produced the evidence
base represented by the 50 sources cited in this manuscript. Sources included arXiv (cs.CR), IEEE/ACM open access, NIST,
...
and $\mathcal{P}$ is the set of conditional governance properties stated for the design.
\end{definition}
...
\subsection{Calibration Assumption and Honest Limits}
\label{ssec:calibration}
PC3 interprets $R$ as a decision score. Treating it as a lower bound on correctness would require
the explicit calibration condition $P(\neg H \mid R=r) \geq r$; positive correlation alone does
not imply this condition. The present component-level experiment does not establish such
calibration for a production LLM. Prompt injection \cite{liu2024formalizing,zou2023universal}
can further inflate apparent reliability while suppressing correctness. L4 controls
(sanitization, instruction--data separation, and output filtering) operate before $R$ is
computed and may reduce this risk, but their ability to preserve or restore calibration must be
measured under attack. This unresolved risk motivates HITL as an architectural primitive.
...
\end{align}
Setting $\mathcal{U}_{\text{auto}} = \mathcal{U}_{\text{HITL}}$ yields their pairwise decision boundary:
\begin{equation}
r^*_{\text{auto/HITL}} = \frac{b + c_{\text{err}} - c_{\text{analyst}} - \lambda}{b + c_{\text{err}}}.
\label{eq:threshold}
\end{equation}
The AUTO/HITL boundary is therefore a function of organizational policy rather than an ad-hoc
constant. A globally optimal three-action policy also requires pairwise comparison with
$\mathcal{U}_{\text{reject}}$; in the current prototype, $\tau_{\text{review}}$ and hard
deny/sensitivity rules remain explicit policy inputs rather than consequences of Eq.~\eqref{eq:threshold}.

\paragraph{Sensitivity to cost parameters.}
For the reference configuration ($b{=}1$, $c_{\text{err}}{=}2$, $c_{\text{analyst}}{=}0.3$,
$\lambda{=}0.3$), $r^*_{\text{auto/HITL}}{=}0.80$. From Eq.~\eqref{eq:threshold},
$c_{\text{err}} \to \infty$ implies $r^*_{\text{auto/HITL}} \to 1$, whereas
$b \gg c_{\text{err}}$ gives
$r^*_{\text{auto/HITL}} \approx 1-(c_{\text{analyst}}+\lambda)/b$.
These limits are interpretable only when the resulting boundary lies in $[0,1]$ and the
calibration and utility assumptions hold.
...
Three observations arise from inspection of Table~\ref{tab:mapping}. First, several technical
controls associated here with selected LGPD obligations overlap with ISO 27001 (A.5.28 for
audit logs, A.5.34 for privacy) and NIST CSF 2.0 (PR.PT-1, PR.DS-1). This overlap may reduce
duplicate implementation work, but it does not establish LGPD compliance or eliminate
LGPD-specific legal and organizational obligations such as lawful basis, purpose limitation,
data minimization, retention, and data-subject rights. Second, the OWASP LLM Top 10
\cite{owasp2025llm} maps primarily to the proposed L4 controls (LLM01/input, LLM02/output,
LLM06/tool access, LLM07/audit, LLM09/retrieval). The mapping is a design traceability aid,
not evidence that the layer decomposition is exhaustive or non-overlapping.
...
alignment. Several technical controls mapped to selected LGPD obligations overlap with
ISO 27001 and NIST CSF 2.0; the heatmap does not establish legal compliance.}
\label{fig:heatmap}
\end{figure}
...
\begin{lemma}[Decision-Trace Completeness under Engineering Invariants]
\label{lem:audit}
...
Under the stated premises, E1 excludes record loss, E2 makes modification of committed records
detectable (Merkle inclusion-proof verification costs $O(\log n)$), and E3 constrains reordering.
The result is conditional: it does not by itself discharge crash consistency, privileged
truncation, rollback, key compromise, or external-checkpoint availability.
\end{proof}

\begin{theorem}[Selected-Control Coverage Monotonicity under Non-Interference]
...
Conflicting requirements (e.g., erasure and sectoral retention) lie outside the
non-interference precondition and require case-specific legal and organizational resolution.
Accordingly, the theorem concerns set coverage in the crosswalk, not legal compliance or
control effectiveness.
\end{proof}

\begin{theorem}[Policy-Derivable AUTO/HITL Boundary]
\label{thm:routing}
Given cost-benefit parameters $(b, c_{\text{err}}, c_{\text{analyst}}, \lambda, c_{\text{miss}})$
and reliability score $R$, Eq.~\eqref{eq:threshold} determines the pairwise indifference
boundary between AUTO and HITL. The complete action selected by Algorithm~\ref{alg:pc3}
also depends on the policy-specified REJECT threshold, deny list, sensitivity set, and signal
disagreement rules.
\end{theorem}
...
\subsection{Prototype Scope and Package Structure}
The public prototype is a four-module surrogate used to exercise ingestion, simulated
LLM/RAG scoring, routing, and local audit verification. It does not yet instantiate the full
six-layer architecture: in particular, the released version does not implement UCO conformance,
multi-hop GraphRAG, a production LLM, or an independently calibrated $\rho$. The modules are:
...
\begin{itemize}[noitemsep,leftmargin=1em]
  ...
  \item \texttt{layer2\_llm\_rag.py} -- simulated LLM/RAG classification derived from
    Random-Forest priors, lexical ATT\&CK retrieval, and a reliability triple.
  \item \texttt{layer3\_hitl.py} -- threshold-based AUTO/HITL/REJECT routing over the
    surrogate reliability score.
  \item \texttt{layer4\_audit.py} -- local HMAC hash-chain append, Merkle batch
...
\emph{Prototype audit store.} The released L6 surrogate writes JSONL output and applies
per-record HMAC-SHA-256 plus local Merkle batch commitments. This demonstrates record
integrity checks in the reference run; it does not implement WORM persistence, HSM-backed
key custody, trusted time, or externally witnessed checkpoints. These are deployment
requirements for the stronger invariants used by Lemma~\ref{lem:audit}.

\emph{Stateless surrogate inference.} Each prototype call is stateless: the reliability
triple, routing decision, and audit record are determined by the current synthetic alert
and simulated classifier output, without reference to session state. This design supports
...
\emph{Production-interface requirement.} The architectural design requires a structured
LLM output with \texttt{verdict}, \texttt{technique}, \texttt{confidence},
\texttt{rationale}, and \texttt{evidence\_path}. The current surrogate does not test malformed
production-LLM output; schema validation and fail-closed routing therefore remain requirements
for the six-layer implementation rather than evaluated properties of this prototype.

\subsection{Experiment Figures}

Fig.~\ref{fig:confusion} shows the row-normalised confusion matrix. The principal
failure modes are intra-family confusions: DoS variants (slowloris, Slowhttptest, GoldenEye)
exchange $\leq 8\%$ of instances among themselves, and Web Attack sub-types (Brute Force,
XSS, SQLi) account for similar intra-family leakage. These classes also carry the lowest
$\gamma'$ values (Fig.~\ref{fig:triple}), which means PC3 routes them predominantly to
HITL -- the intended dependability behaviour: classification uncertainty is surfaced to an
analyst rather than auto-resolved.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{E4_routing_ieee.png}
\caption{E4 -- PC3 routing distribution per attack class (attack flows only, seed 42).
HIGH-confidence classes (DDoS, DoS Hulk) are auto-resolved at $\geq 53\%$; semantically
ambiguous or low-frequency classes (PortScan, SSH-Patator, Bot) are escalated to HITL
at rates $\geq 79\%$. The dashed vertical line marks the AUTO share threshold (70\%).}
\label{fig:routing}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{E6_triple_ieee.png}
\caption{E6 -- Mean reliability triple $\langle\theta,\sigma,\gamma'\rangle$ per class
(test set, seed 42). Self-consistency $\sigma$ is uniformly high across classes because
the classifier prior is stable at temperature 0.45. Confidence $\theta$ tracks per-class
recall. Adversarial groundedness $\gamma'$ is the principal differentiator: DDoS reaches
$\gamma'{\approx}0.78$ while PortScan drops to ${\approx}0.72$ and Bot to ${\approx}0.26$,
driving the routing distribution in Fig.~\ref{fig:routing}. The dashed line marks
$r^*_{\mathrm{auto}}{=}0.80$.}
\label{fig:triple}
\end{figure}
...
(downtime cost per minute $\times$ expected incident duration); Eq.~\eqref{eq:threshold}
allows the AUTO/HITL boundary to be recomputed under those domain-specific values without
modifying the routing algorithm. Subject to implementation of the stated persistence and
access-control requirements, the L6 record can support evidence collection for incident
communication. Resolution CD/ANPD 15/2024 requires communication to the ANPD and affected
data subjects within three business days when its applicability conditions are met
\cite{anpd15}; it does not impose a generic 72-hour rule. P2 states only that adding a
non-interfering mapped control does not reduce selected requirement coverage.
...
We compare SHIELD-AI against five adjacent governance artifacts selected for their coverage
of AI/LLM governance and cybersecurity: ENISA Multilayer Framework \cite{enisa2023multilayer},
...
SHIELD-AI's design. Because the criteria derive from this paper's research questions and the
coding is author-conducted, the scores should not be interpreted as an independent ranking.
...
The ``Formal properties'' row (C5) warrants particular attention from the TDSC perspective.
Decision-trace completeness under E1--E3 (P1), selected-control coverage monotonicity under
non-interference (P2), and the policy-derived AUTO/HITL boundary (P3) structure the proposed
assurance argument. Their assumption burden and implementation status must be assessed
alongside their formal statements; the comparison does not establish certification or
regulatory superiority over the other artifacts.
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Author-scored scenario walkthrough (Likert 1--5). Scores illustrate internal design
coverage and are not an independent validation.}
...
performance, routing distribution, and local audit checks. In this single seed/split, the
surrogate produces F1~$0.984$ versus $0.985$ and AUC~$0.998$ versus $0.999$ for the Random
Forest baseline. No inferential or non-inferiority conclusion follows from these point
estimates. The rule-based detector has a substantially higher observed FPR (0.375 versus
0.004), but also a much lower F1 (0.516 versus 0.984), so this is not a matched-F1 comparison.
The surrogate additionally emits reliability signals, routing decisions, and a locally
...
SHIELD-AI surrogate & 0.984 & 0.004 & 0.998 & 0.497 & 69/30/1\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key observations.} $\gamma'$ (mean 0.575, $p_{10}$=0.255, $p_{90}$=0.766) is the
principal routing differentiator (Fig.~\ref{fig:triple}): DDoS reaches $\gamma'{\approx}0.78$
while PortScan drops below 0.72, producing the class-stratified routing shown in
Fig.~\ref{fig:routing}. L6 audit latency averages 0.014~ms/record; chain re-derivation passes
for all 15{,}000 records, and tamper tests detect a single bit-flip in any record.

\textbf{Per-class routing.} Fig.~\ref{fig:routing} shows the PC3 distribution across all
attack classes. High-confidence families (DDoS, DoS Hulk) reach AUTO rates $\geq 53\%$;
rare and semantically ambiguous classes (PortScan, SSH-Patator, Bot, Infiltration) are
escalated to HITL at rates $\geq 79\%$, consistent with the intended dependability behaviour.
Estimates for rare classes are especially uncertain given small support counts.
...
+ simulated $\rho$ signal  & 0.984 & 0.004 & 29.7 & 1.00 \\
\midrule
Four-module surrogate  & 0.984 & 0.004 & 29.7 & 1.00 \\
\bottomrule
\end{tabular}}
\end{table}

In this run, adding the local audit path leaves F1/FPR unchanged; this observation does not
imply zero compute, storage, or durability cost. PC3 routes 30.7\% of cases to HITL in the
triple-only configuration. The row labelled ``GraphRAG'' uses a simulated additional
confidence signal rather than the full UCO/multi-hop engine specified in
Section~\ref{ssec:graphrag}; it reduces the observed HITL share to 29.7\%. The F1 difference
between RF-only and the surrogate pipeline (0.985 versus 0.984) is a point-estimate result.
Repeated seeds, paired tests, and confidence intervals are required to assess variance or
non-inferiority.
...
(a DOI-pinned release remains future work; see OP7).
...
A cross-layer supply-chain threat also applies: a compromised CTI feed at L1 can
inject false IOC associations that propagate into the L2 Knowledge Graph, inflating
$\rho$ for malicious edges and bypassing the adversarial-uncertainty HITL rule.
Mitigation requires content-hash pinning of all corpus snapshots at ingestion time
and periodic cross-validation of graph edges against a second authoritative source.
...
A further conclusion threat is surrogate validity: the four-module prototype exercises
the routing and audit logic but not the UCO conformance, multi-hop GraphRAG, or
production-LLM path; quantitative results therefore speak to the surrogate's behavior
rather than to the full six-layer architecture, and any extrapolation to field performance
requires independent evaluation on live traffic.
...
The prototype suggests that governance controls can be integrated into a common processing
pipeline; it does not establish simultaneous legal compliance. Practitioners should establish
auditable decision traces and meaningful human oversight before scaling LLM/RAG autonomy,
a sequencing that inverts the ``add AI first, govern later''
...
estimates before production rollout. Third, \emph{monitor HITL workload and outcomes}:
escalation rate, queue delay, analyst agreement, overrides, and error rates conditional on
AUTO/HITL/REJECT should be evaluated together. Escalation rate alone cannot diagnose
overconfidence or corpus insufficiency without ground-truth and calibration evidence.
Fourth, \emph{maintain the RAG corpus actively}: the $\gamma'$ signal depends on the
freshness and coverage of the ATT\&CK knowledge base; a corpus that has not been updated
for major ATT\&CK or ATLAS releases will suppress $\gamma'$ for recently discovered
techniques, increasing HITL load precisely for the classes where rapid automated
triage carries the highest operational value.

\subsection{Dependability Properties and Deployment Obligations}
The three conditional properties (P1 decision-trace completeness, P2 selected-control
coverage monotonicity, and P3 policy-derived AUTO/HITL boundary) define operational
assurance obligations rather than field-validated guarantees.
P1 specifies the fields needed for post-incident reconstruction when E1--E3 are implemented;
the released surrogate does not yet discharge all three invariants.
P2 provides set-coverage monotonicity under non-interference; it does not eliminate legal
review or the need to re-evaluate implementation effectiveness when mappings change.
...
Nine limitations are explicitly acknowledged:
...
(A3) L6 staffing assumes mid-to-large SOCs; a ``L6-light'' variant for small SOCs -- using
sampling-based audit rather than full WORM persistence -- is an open engineering problem.
(A4) LLM version drift shifts the empirical distribution of $(\theta, \sigma, \gamma, \rho)$;
each major model update requires re-running the reliability regression suite before promotion.
...
(A8) The public experiment is a four-module surrogate and does not instantiate the six-layer
UCO/GraphRAG architecture or a production LLM.
(A9) Quantitative results use one synthetic dataset generation, one 75/25 split, and seed 42;
they do not quantify training/sampling uncertainty or establish non-inferiority.
...
prompt); and L6 audit ($O(1)$ HMAC per record, asynchronous $O(B)$ Merkle-tree construction
for a batch of $B$ leaves).
...
Failover behavior under L4 unavailability is governed by the PC3 REJECT branch:
when the LLM path is unreachable, the deny-list and sensitivity rules still route
high-severity alerts to HITL, preserving analyst visibility even in degraded mode.
This fail-closed design satisfies the availability--safety trade-off required in
critical-infrastructure SOC deployments where partial operation is preferable to
silent failure.
...
Key metrics to instrument include per-class HITL rate, analyst override frequency,
time-to-decision, and post-incident L6 reconstruction completeness; together they would
establish whether surrogate thresholds transfer to real traffic distributions and whether
the Merkle-chained audit log supports regulatory reporting without manual reconstruction.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{E2_confusion_ieee.png}
\caption{E2 -- Row-normalised confusion matrix for the SHIELD-AI surrogate across 15 classes
($n{=}15{,}000$ test flows, seed 42). Diagonal values are per-class recall.
Confusions concentrate within attack families (DoS variants; Web Attack sub-types).
Rare classes (Infiltration, Heartbleed, SQLi) show the largest off-diagonal mass and are
correspondingly routed to HITL at rates $\geq 64\%$ (Fig.~\ref{fig:routing}).}
\label{fig:confusion}
\end{figure*}
...
SHIELD-AI specifies a dual-use GenAI taxonomy, a risk model with reliability
...
cross-standard control mapping including selected LGPD obligations. We formalize the
framework as a 6-tuple, derive the AUTO/HITL boundary from cost-benefit parameters, and state
conditional properties for decision-trace completeness, selected-control coverage, and
policy-derived routing. Interpreting the reliability score probabilistically requires empirical
calibration, especially under prompt injection. The healthcare and critical-infrastructure
scenarios illustrate prospective applicability but do not demonstrate transferability. The
synthetic component-level experiment demonstrates execution of a four-module surrogate
(F1~0.984; AUC~0.998) and local audit-chain verification; it does not validate a production
LLM, UCO/GraphRAG implementation, field effectiveness, or adversarial robustness.
...
systematic failure analysis. The three formal properties (P1-P3) organize the assurance
argument, subject to their stated premises: P1 gives decision-trace completeness when E1--E3
hold; P2 gives monotonic selected-requirement coverage under non-interference; and P3 derives
the AUTO/HITL boundary from policy parameters. They do not, by themselves, establish
production durability, legal compliance, global three-action optimality, certification
readiness, or regulatory acceptance. Independent implementation and evaluation are required
to discharge those assurance obligations.

\subsection*{Limitations and Forward View}
Nine limitations are acknowledged (A1--A9, Section~\ref{sec:discussion}). The most
consequential for the present evidence is A8: the public experiment is a four-module
surrogate rather than an implementation of the complete six-layer architecture. A4 further
notes that production-LLM version drift would shift the empirical distribution of the
reliability signals and require recalibration. These limitations motivate OP1, OP8, and OP9.
The nine-item research agenda (OP1--OP9)
